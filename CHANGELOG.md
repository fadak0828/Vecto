...
## [0.17.1] - 2026-06-24 — 접두어 방식 중복 생성·결과 화면 수정

주소창 접두어 방식(`만들기.좌표.to`)의 라이브 버그 2건 수정.

### Fixed
- **슬러그 중복 생성** — GET 방식이라 브라우저 prefetch·새로고침으로 같은 요청이 여러 번 들어와 슬러그가 2개 만들어지던 문제. 같은 IP 가 같은 원본 URL 로 만든 미만료 슬러그가 있으면 새로 만들지 않고 재사용하도록 멱등화. (동시 요청 race 는 잔여 위험으로 남김)
- **결과 화면** — 생성 후 곧장 원본 URL 로 튕겨 만들어진 단축 주소·QR 을 못 보던 문제. 이제 `좌표.to/?created=<slug>` 로 보내 홈 결과 카드(단축 주소 + QR + 복사)를 표시. `HeroInteractive` 가 `?created` 쿼리를 읽어 기존 결과 UI 를 재사용하고, 표시 후 쿼리는 제거.
