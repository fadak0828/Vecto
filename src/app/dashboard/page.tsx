        ...
        {!namespace ? (
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-3xl font-bold">좌표.to/{namespace.name}</h1>
                <a
                  href={`/${namespace.name}`}
                  target="_blank"
            ...
            <div className="space-y-2 mb-8">
              {links.length === 0 && (
                <div className="py-8 text-center text-[var(--muted)] bg-[var(--surface)] rounded-xl border border-[var(--border)]">
                  <p className="mb-1">아직 링크가 없습니다</p>
                  <p className="text-sm">아래에서 첫 번째 링크를 추가하세요.</p>
                </div>
              )}
              {links.map((link) => (
                <div
                  key={link.id}
                  className="flex items-center gap-3 p-4 bg-[var(--surface)] rounded-xl border border-[var(--border)]"
                >
